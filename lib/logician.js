...
function valueFromRule(rule, vector) {
	...
	if (typeof(rule.base) == "number") accumulator = rule.base;
	var parameterName;
	if (vector != null)
		for (parameterName in vector)
			if (rule.multipliers instanceof Object &&
					typeof(rule.multipliers[parameterName]) == "number")
				accumulator += rule.multipliers[parameterName] * vector[parameterName];
	if (typeof(rule["min"]) == "number" && accumulator < rule["min"])
		accumulator = rule["min"];
	if (typeof(rule["max"]) == "number" && accumulator > rule["max"])
