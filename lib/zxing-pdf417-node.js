...
//
// Ported to Node by Frank Trampe 2017
...
ZXing.BitmapLuminanceSourcePNG = function (data) {
	var pngr = pngjs.PNG.sync.read(data);
	ZXing.BitmapLuminanceSource.call(this, Uint8ClampedArray.from(pngr.data), pngr.width, pngr.height);
};

ZXing.BitmapLuminanceSourcePNGCustom = function (data, opts) {
	var pngr = pngjs.PNG.sync.read(data);
	ZXing.BitmapLuminanceSourceCustom.call(this, Uint8ClampedArray.from(pngr.data), pngr.width, pngr.height, opts);
};

ZXing.BitmapLuminanceSourcePNGFile = function (bitmapfile) {
	var data = fs.readFileSync(bitmapfile);
	ZXing.BitmapLuminanceSourcePNG.call(this, data);
};

ZXing.BitmapLuminanceDumpPNG = function (img) {
	if ('debugBitmap' in img) {
		return pngjs.PNG.sync.write({data: img.debugBitmap, width: img.get_Width(), height: img.get_Height()});
	}
	return null;
};

ZXing.BitmapLuminanceSource = function (bitmap, w, h) {
	ZXing.BitmapLuminanceSourceCustom.call(this, bitmap, w, h, {});
};

ZXing.BitmapLuminanceSourceCustom = function (bitmap, w, h, opts) {

    var debug = 1; // typeof window != 'undefined' && window.__debug === true;
    ...
    if (typeof bitmap == 'number') {
        ...
        if (bitmap instanceof Uint8ClampedArray) {
            ...
            height = h;
            data = bitmap;
        } else if (0 && bitmap instanceof ImageData) {
        ...
        //console.time("luminances")
				...
				var greenw = 38550;
				var bluew = 19562
				if (opts != null && opts != undefined && 'channel_weights' in opts) {
					redw = opts.channel_weights.red;
					greenw = opts.channel_weights.green;
					bluew = opts.channel_weights.blue;
				}
				// console.log("Weights: " + redw + " " + greenw + " " + bluew + ".");
        for (var y = 0; y < height; y++) {
            var strideOffset = y * stride;

            var maxIndex = (4 * width) + strideOffset;
            for (var x = strideOffset; x < maxIndex; x += 4) {
                var luminance = ((redw * data[x] + greenw * data[x + 1] + bluew * data[x + 2]) >> 16);
                if ('quadraticness' in opts && opts.quadraticness != null) {
                  if (opts.quadraticness < 0) {
                    var maxv = 255;
                    var qv = luminance * luminance / maxv;
                    var newlum = Math.round(luminance * (1 - Math.abs(opts.quadraticness)) + qv * Math.abs(opts.quadraticness));
                    if (y < 8/* && x == strideOffset + Math.floor((maxIndex - strideOffset) / 2)*/) {
                      // console.log("IL: " + luminance + "; QV: " + qv + "; OW: " + (1 - Math.abs(opts.quadraticness)) + "; OL: " + newlum + ".");
                    }
                    luminance = newlum;
                  }
                }
                if ('quarticness' in opts && opts.quarticness != null) {
                  if (opts.quarticness < 0) {
                    var maxv = 255;
                    var qv = luminance * luminance / maxv * luminance / maxv * luminance / maxv;
                    var newlum = Math.round(luminance * (1 - Math.abs(opts.quarticness)) + qv * Math.abs(opts.quarticness));
                    if (y < 8/* && x == strideOffset + Math.floor((maxIndex - strideOffset) / 2)*/) {
                      // console.log("IL: " + luminance + "; QV: " + qv + "; OW: " + (1 - Math.abs(opts.quarticness)) + "; OL: " + newlum + ".");
                    }
                    luminance = newlum;
                  }
                }
                if ('hexicness' in opts && opts.hexicness != null) {
                  if (opts.hexicness < 0) {
                    var maxv = 255;
                    var qv = luminance * luminance / maxv * luminance / maxv * luminance / maxv * luminance / maxv * luminance / maxv;
                    var newlum = Math.round(luminance * (1 - Math.abs(opts.hexicness)) + qv * Math.abs(opts.hexicness));
                    if (y < 8/* && x == strideOffset + Math.floor((maxIndex - strideOffset) / 2)*/) {
                      // console.log("IL: " + luminance + "; QV: " + qv + "; OW: " + (1 - Math.abs(opts.hexicness)) + "; OL: " + newlum + ".");
                    }
                    luminance = newlum;
                  }
                }
                if ('threshold' in opts && opts.threshold != null) {
                  if (opts.threshold >= 0 && opts.threshold <= 255) {
                    if (luminance <= opts.threshold) luminance = 0;
                    else luminance = 255;
                  }
                }
                var alpha = data[x + 3];
		if (alpha != 255) console.log("Not flat.");
		...
		if (!('get_Width' in this)) this.get_Width = function () { return w; };
		if (!('get_Height' in this)) this.get_Height = function () { return h; };
		if (!('get_Matrix' in this)) this.get_Matrix = function () { return this.luminances; };
