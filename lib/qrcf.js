/*
  ...
  Ported to node.js by Frank Trampe 2017 
...
function AlignmentPatternFinder( qrcode, image,  startX,  startY,  width,  height,  moduleSize,  resultPointCallback)
{
	...
	// console.log("X, Y, W, H, MS:", startX, startY, width, height, moduleSize);
	...
	this.handlePossibleCenter=function( stateCount,  i,  j)
		{
			...
			if (!isNaN(centerI))
			{
				...
				if (!isNaN(centerJ))
				{
					...
					var eVertC = 0;
					var tvp = NaN;
					if (centerJ + estimatedModuleSize < qrcode.width) tvp = this.crossCheckVertical(Math.floor(centerI), Math.floor(centerJ + estimatedModuleSize), stateCount[2], stateCountTotal);
					if (!isNaN(tvp)) eVertC++;
					// console.log("V:", tvp);
					tvp = NaN;
					if (centerJ - estimatedModuleSize >= 0) this.crossCheckVertical(Math.floor(centerI), Math.floor(centerJ - estimatedModuleSize), stateCount[2], stateCountTotal);
					if (!isNaN(tvp)) eVertC++;
					// console.log("V:", tvp);
					// console.log("Checked verticals from y = " + (Math.floor(centerI)).toString() + " at x values " + (Math.floor(centerJ + estimatedModuleSize)).toString() + " and " + (Math.floor(centerJ - estimatedModuleSize)).toString() + ".");
					// console.log("Extra vertical count:", eVertC);
					...
					if ((currentState & 1) == 0)
					{
						...
						if (currentState == 4)
						{
							...
							if (this.foundPatternCross(stateCount))
							{
								...
								if (confirmed)
								{
									// console.log("Possible center (x, y, c):", j, i, confirmed);
									...
									if (this.hasSkipped)
									{
										...
										// console.log("Consider it " + (done ? "done" : "undone" ) + ".");
									}
									else
									{
...
module.exports = exports = qrcodereader;
